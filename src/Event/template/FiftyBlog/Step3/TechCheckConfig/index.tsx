...
import TemplateFields from 'Event/template/FiftyBlog/Step3/TechCheckConfig/TemplateFields'
...
import TechCheck from 'Event/template/FiftyBlog/Step3/TechCheck'
...
const DEFAULT_TECH_CHECK_PROPS: NonNullable<FiftyBlog['techCheck']> = {
  buttonText: '',
...
export type FiftyBlogTechCheckTemplateProps = NonNullable<
  FiftyBlog['techCheck']
>

type TechCheckTemplatePropSetter = <
  K extends keyof FiftyBlogTechCheckTemplateProps
>(
  key: K,
) => (value: FiftyBlogTechCheckTemplateProps[K]) => void
...
function useTemplateTechCheckProps() {
  const [techCheck, setTechCheck] = useState<FiftyBlogTechCheckTemplateProps>(
  ...
  useEffect(() => {
    ...
    const fetchedTechCheck = event?.template
      ?.techCheck as FiftyBlog['techCheck']
