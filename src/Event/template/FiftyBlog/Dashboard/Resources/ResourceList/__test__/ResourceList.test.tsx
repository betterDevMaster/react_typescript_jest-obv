...
import {fakeResource} from 'Event/template/FiftyBlog/Dashboard/Resources/ResourceList/__utils__/factory'
...
it('should add a new resource', async () => {
  ...
  const {findByLabelText, findAllByLabelText} = await goToFiftyBlogConfig({
    event,
  })
...
it('should update resources title', async () => {
  ...
  const {findByLabelText} = await goToFiftyBlogConfig({
    event,
  })
