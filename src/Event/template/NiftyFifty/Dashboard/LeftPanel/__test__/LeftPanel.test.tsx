...
it('should render left panel', async () => {
  const {findByLabelText, queryByText} = await goToDashboardConfig({
    event: fakeEvent({template: fakeNiftyFifty(), logo: null}),
  })
...
it('should render left panel', async () => {
  const event = fakeEvent({template: fakeNiftyFifty(), logo: null})
  ...
  await wait(async () => {
    ...
    // expect(await findByLabelText('left panel bar text color')).toHaveStyle(
