...
import {goToNiftyFiftyConfig} from 'Event/template/NiftyFifty/__utils__/go-to-fifty-blog-config'
...
it('should add a new resource', async () => {
  const dashboard = fakeNiftyFifty({
    resourceList: {
  ...
  const {findByLabelText, findAllByLabelText} = await goToNiftyFiftyConfig({
    event,
  })
...
it('should update resources title', async () => {
  ...
  const {findByLabelText} = await goToNiftyFiftyConfig({
    event,
  })
