...
import TemplateFields from 'Event/template/NiftyFifty/Step3/TechCheckConfig/TemplateFields'
...
const DEFAULT_TECH_CHECK_PROPS: NonNullable<NiftyFifty['techCheck']> = {
  buttonText: '',
...
export type NiftyFiftyTechCheckTemplateProps = NonNullable<
  NiftyFifty['techCheck']
>

type TechCheckTemplatePropSetter = <
  K extends keyof NiftyFiftyTechCheckTemplateProps
>(
  key: K,
) => (value: NiftyFiftyTechCheckTemplateProps[K]) => void
...
function useTemplateTechCheckProps() {
  const [techCheck, setTechCheck] = useState<NiftyFiftyTechCheckTemplateProps>(
  ...
  useEffect(() => {
    ...
    const fetchedTechCheck = event?.template
      ?.techCheck as NiftyFifty['techCheck']
