...
export default function NameAppendageConfig() {
    ...
    const [nameAppendagesList, setNameAppendagesListList] = useState<NameAppendage[]>(name_appendages)
    const [editing, setEditing] = useState<NameAppendage | null>(null)

    const add = () => setAdding(true)
    const closeAddDialog = () => setAdding(false)
    const setEditDialog = (nameAppendage : NameAppendage) => setEditing(nameAppendage)
    const closeEditDialog = () => setEditing(null)

    const changeNameAppendagesList = (name_appendage: NameAppendage[]) => setNameAppendagesListList(name_appendage)
    ...
    return (
        <Layout>
            <Page>
                ...
                <NameAppendageUpdateDialog nameAppendage={editing} nameAppendages={nameAppendagesList} setNameAppendages={changeNameAppendagesList} onClose={closeEditDialog}/>

                <NameAppendageAddButton openAdd={add} />
                <NameAppendageListTable setEditing={setEditDialog} nameAppendages={nameAppendagesList} setNameAppendages={changeNameAppendagesList} />
            </Page>
        </Layout>
    )
}