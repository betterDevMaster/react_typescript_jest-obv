...
export default function EmojiesSelector( props: {
    ...
    const ToggleButton = () => {
        return (
            <>
                <StyledToggleButton  onClick={()=>toggleButtonHandle()} > {showEmojies? "Hide label emojies list" : props.selected? "Selected emoji: "+props.selected : "Select label emoji"}  </StyledToggleButton>
    ...
    function GenerateEmojiWithButton ( props: {
        ...
        if(props.emoji === props.selected){
            return (<Button variant="contained" color="primary" onClick={() => { props.onSelect() }}>{props.emoji}</Button>)
        }else{
            return (<Button onClick={() => { props.onSelect() }}>{props.emoji}</Button>)
        }
    }
                ...
                <EmojiSelectorGrid container>
                    {emojiesList.map((emoji, index) => (
                        <Grid item sm={1} key={index}>
                            <GenerateEmojiWithButton emoji={emoji} onSelect={() => HandleSelect(emoji)} selected={props.selected}/>
                        </Grid>
                    ))}
                </EmojiSelectorGrid>
            </EmojiSelectorContainer>
    ...
    return (
        <>
            {showEmojies ? <ShowEmojiSelector /> : <HideEmojiSelector/>}
        </>
    )
}
