    ...
    if(props.rules.length){
        props.rules.map((value: any, index: number) => {
            ...
            if(value.source === "Nested Rule"){
                generatedText += value.source + " with " + value.rules.length + " rules ";
            }else{
        ...
        generatedText = generatedText.charAt(0) + generatedText.substring(1).toLowerCase()
    }else{
    ...
    if(props.cropText === true && generatedText.length > 110){
        return (
            <>
                ...
                <a href={"javascript:void(0)"} onClick={props.seeMoreCallback}>(See more)</a>
            </>
        )
    }
